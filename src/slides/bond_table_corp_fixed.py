"""
Слайд «Рублевые облигации — корпоративные облигации с фиксированным купоном».

Отличается от флоатеров тем, что:
- Купон — число (из CSV), а не формула из yaml
- Доходность и дюрация отображаются (MOEX их считает для фиксов корректно)
- Есть колонка "Дата оферты" — у многих корпоратов есть put/call опционы

Бумаг много (~144 в образце) → пагинация на слайды по N штук.
На каждой странице — полная шапка секции и колонок, как договорились.
"""
...
SECTION_TITLE_CORP_FIXED = "Корпоративные облигации с фиксированным купоном"

# 12 колонок: 11 данных + "В перечне"
CORP_FIXED_COLUMNS = [
    TableColumn("№", TABLE_CORP_FIXED_COL_WIDTHS_CM[0], align="center"),
    TableColumn("Выпуск", TABLE_CORP_FIXED_COL_WIDTHS_CM[1], align="center"),
    TableColumn("ISIN", TABLE_CORP_FIXED_COL_WIDTHS_CM[2], align="center"),
    TableColumn("Дата\nпогашения", TABLE_CORP_FIXED_COL_WIDTHS_CM[3], align="center"),
    TableColumn("Дата\nоферты", TABLE_CORP_FIXED_COL_WIDTHS_CM[4], align="center"),
    TableColumn("Купон,\n% год.", TABLE_CORP_FIXED_COL_WIDTHS_CM[5], align="center"),
    TableColumn("Цена, %\nот ном.", TABLE_CORP_FIXED_COL_WIDTHS_CM[6], align="center"),
    TableColumn("Дох-ть к\nпогаш.,\n% год.", TABLE_CORP_FIXED_COL_WIDTHS_CM[7], align="center"),
    TableColumn("Мод.\nдюр.", TABLE_CORP_FIXED_COL_WIDTHS_CM[8], align="center"),
    TableColumn("Объём\nвыпуска,\nмлн ₽", TABLE_CORP_FIXED_COL_WIDTHS_CM[9], align="center"),
    TableColumn("Период.\nкупона", TABLE_CORP_FIXED_COL_WIDTHS_CM[10], align="center"),
    TableColumn("В\nперечне", TABLE_CORP_FIXED_COL_WIDTHS_CM[11], align="center"),
]

CORP_FIXED_ROWS_PER_PAGE = 24


def _bond_to_row(i: int, b: Bond, offer_override: Optional[str], highlighted: set) -> List[str]:
    """Одна облигация → строка для таблицы корп-фиксов."""
    if offer_override:
...
def _render_one_page(
    ...
    """Один слайд-страница с подмножеством бумаг."""
    ...
    rows = [
        _bond_to_row(start_number + i, b, offer_dates.get(b.isin), highlighted)
...
def render_corp_fixed(
    prs: PresentationType,
    bonds: List[Bond],
    offer_dates: Optional[Dict[str, str]] = None,
    section_title: str = SECTION_TITLE_CORP_FIXED,
    report_date=None,
    rows_per_page: int = CORP_FIXED_ROWS_PER_PAGE,
    highlighted: Optional[set] = None,
) -> None:
    """
    Добавляет серию слайдов с таблицей корпоративных фиксов.

    Args:
        prs: презентация
        bonds: облигации
        offer_dates: словарь ISIN → дата оферты (DD.MM.YYYY, из yaml)
        section_title: заголовок секции
        report_date: дата отчёта
        rows_per_page: сколько строк на слайд
